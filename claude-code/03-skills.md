...
## 为什么用 Skill，而不是都写进 CLAUDE.md？

**CLAUDE.md 每次对话都全量加载**，里面的每一行都占用上下文，不管这次对话用不用得上。如果你把 PRD 写作规范、竞品分析框架、周报模板全堆进 CLAUDE.md，Claude 在你只是想让它帮你改个错别字时，也要把那几百行规范一起读进来——既浪费 token，也可能干扰它的判断。

Skill 的优势在于**按需加载**：不触发就不占用上下文，触发时才把完整内容注入对话。这让你可以放心地把各种专项知识组织成独立模块，积累再多也不影响日常使用。

| | CLAUDE.md | Skill |
|--|-----------|-------|
| 加载时机 | 每次对话启动时全量加载 | 触发时才加载 |
| 适合放什么 | 始终需要的背景：产品定位、团队约定、写作风格 | 专项场景才用的知识：PRD 模板、代码审查规范 |
| 内容增多时 | 每次都消耗更多上下文 | 不影响，不触发不加载 |
| 可复用性 | 项目级，需手动复制到其他项目 | 可放 `~/.claude/skills/` 全局复用 |

**一句话判断**：这条内容是不是每次对话都需要？是 → 写 CLAUDE.md；否 → 做成 Skill。
